...
class App(QWidget):
    def __init__(self, prop_net, fuse_net, s2m_ctrl:S2MController, fbrs_ctrl:FBRSController, 
                    images, masks, num_objects, mem_freq, mem_profile, save_path):
        super().__init__()
    ...
    def save(self):
        ...
        mask_dir = path.join(self.save_path, 'mask')
        overlay_dir = path.join(self.save_path, 'overlay')
...
if __name__ == '__main__':
    ...
    with torch.cuda.amp.autocast(enabled=not args.no_amp):
        ...
        if args.images is not None:
            images = load_images(f'{args.images}/image', args.resolution if args.resolution > 0 else None)
        ...
        ex = App(prop_model, fusion_model, s2m_controller, fbrs_controller, images, masks, num_objects, args.mem_freq, args.mem_profile, args.images)
        sys.exit(app.exec_())